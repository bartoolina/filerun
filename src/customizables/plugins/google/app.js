		...
		Ext.Ajax.request({
			...
			success: function(req) {
				...
				if (this.sendFileResults.success) {
					...
					this.popup.close();
					if (this.sendFileResults.msg) {window.parent.FR.UI.feedback(this.sendFileResults.msg, 'error');}
					window.parent.FR.UI.popups[windowId].close();
				}
			},
			scope: this
	...
	retrieveFile: function(fileId) {
		...
		Ext.Ajax.request({
			...
			success: function(req) {
				...
				if (rs.msg) {window.parent.FR.UI.feedback(rs.msg, rs.success ? 'success': 'error');}
				if (rs.success) {
